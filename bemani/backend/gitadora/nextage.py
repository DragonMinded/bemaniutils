...
from bemani.backend.gitadora.musiclists.nextagemusic import (
    MUSICLIST_NEXTAGE,
    MUSICLIST_NEXTAGE_OMNIMIX,
)
...
class GitadoraNextage(
    ...
    def handle_nextage_gameinfo_request(self, request: Node) -> Node:
        ...
        extra_music = [
            ...
            1916,
            2289,
            2631,  # DD13 and encores.
            1704,
            1811,
            2121,
            2201,
            2624,  # Soranaki and encores.
            1907,
            2020,
            2282,
            2341,
            2666,  # Stargazer and encores.
        ]
        ...
        for (mid, _plays) in self.data.local.music.get_hit_chart(
            self.game, self.music_version, 41
        ):
        ...
        jubeat_omiyage_challenge_data.add_child(
            Node.s32("linkage_id", jubeat_omiyage_challenge_event[0])
        )
        jubeat_omiyage_challenge_data.add_child(
            Node.u64("start_date", jubeat_omiyage_challenge_event[1])
        )
        jubeat_omiyage_challenge_data.add_child(
            Node.u64("end_date", jubeat_omiyage_challenge_event[2])
        )
    ...
    def format_profile(self, userid: UserID, profile: Profile) -> Node:
        ...
        score.add_child(
            Node.s32("one_day_play_cnt", thanksgiving_dict.get_int("one_day_play_cnt"))
        )
        score.add_child(
            Node.s32(
                "one_day_lottery_cnt", thanksgiving_dict.get_int("one_day_lottery_cnt")
            )
        )
        ...
        score.add_child(
            Node.u64("play_date_ms", thanksgiving_dict.get_int("play_date_ms"))
        )
        lottery_result = Node.void("lottery_result")
        thanksgiving.add_child(lottery_result)
        lottery_result.add_child(
            Node.u64("unlock_bit", thanksgiving_dict.get_int("unlock_bit"))
        )
