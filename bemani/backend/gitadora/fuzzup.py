...
class GitadoraFuzzUp(
    ...
    def handle_fuzzup_shopinfo_regist_request(self, request: Node) -> Node:
        """
        ...
        """
        root = Node.void("fuzzup_shopinfo")
    ...
    def handle_fuzzup_gameinfo_get_request(self, request: Node) -> Node:
        root = Node.void("fuzzup_gameinfo")
        ...
        extra_music = [
            ...
            2814, # Heptagram
            2783, # Saiph
            2848, # LUCID NIGHTMARE
            2860, # Mobius
            2844, # Under The Shades Of The Divine Ray
        ]
        ...
        for extra_music_id in extra_music:
            ...
            music.add_child(Node.u8("get_border", 127))
        ...
        data_ver_limit.add_child(Node.s32("term", 0))
        ...
        bonus = Node.void("bonus")
        root.add_child(livehouse)
        livehouse.add_child(event_list)
        event_list.add_child(event)
        event.add_child(Node.bool("is_open", False))
        event.add_child(Node.u8("term", 0))
        event.add_child(Node.string("livehouse_name", "Xepher"))
        event.add_child(Node.u64("start_date_ms", 0))
        event.add_child(Node.u64("end_date_ms", 0))
        event.add_child(reward_list)
        reward_list.add_child(reward)
        reward.add_child(Node.s32("reward_id", 0))
        reward.add_child(Node.s32("reward_kind", 0))
        reward.add_child(Node.s32("reward_itemid", 0))
        reward.add_child(Node.s32("unlock_border", 0))
        event.add_child(Node.s32("requirements_musicid", 0))
        event.add_child(Node.s32("member_table", 0))
        livehouse.add_child(bonus)
        bonus.add_child(Node.u8("term", 0))
        bonus.add_child(Node.s32("stage_bonus", 0))
        bonus.add_child(Node.s32("charm_bonus", 0))
        bonus.add_child(Node.u64("start_date_ms", 0))
        bonus.add_child(Node.u64("end_date_ms", 0))
    ...
    def handle_fuzzup_cardutil_regist_request(self, request: Node) -> Node:
        root = Node.void("fuzzup_cardutil")
    ...
    def handle_fuzzup_cardutil_check_request(self, request: Node) -> Node:
        root = Node.void("fuzzup_cardutil")
        # get player infomation
        refid = request.child_value("player/refid")
    ...
    def handle_fuzzup_gametop_request(self, request: Node) -> Node:
        ...
        if root is None:
            root = Node.void("fuzzup_gametop")
        return root

    def handle_fuzzup_gameend_request(self, request: Node) -> Node:
        refid = request.child_value("player/refid")
    ...
    # HDD crash
    def handle_fuzzup_assert_report_request(self, request: Node) -> Node:
        root = Node.void("fuzzup_assert_report")
    ...
    def handle_fuzzup_playablemusic_request(self, request: Node) -> Node:
        ...
        if self.omnimix == True:
            all_songs = MUSICLIST_FUZZUP_OMNIMIX
        else:
            all_songs = MUSICLIST_FUZZUP
    ...
    def handle_fuzzup_processing_report_request(self, request: Node) -> Node:
        return Node.void("fuzzup_processing_report")
    ...
    def format_profile(self, userid: UserID, profile: Profile) -> Node:
        ...
        # livehouse
        livehouse = Node.void("livehouse")
        player.add_child(livehouse)
        score_list = Node.void("score_list")
        livehouse.add_child(score_list)
        score_list.add_child(score)
        score.add_child(Node.u8("term", 0))
        score.add_child(Node.s32("reward_id", 0))
        score.add_child(Node.s32("unlock_point", 0))
        score.add_child(Node.s32("chara_id_guitar", 0))
        score.add_child(Node.s32("chara_id_bass", 0))
        score.add_child(Node.s32("chara_id_drum", 0))
        score.add_child(Node.s32("chara_id_other", 0))
        score.add_child(Node.s32("leader", 0))
        score_list.add_child(Node.s32("last_livehouse", 0))
