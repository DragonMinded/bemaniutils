...
class SoundVoltexExceedGear(
        ...
        return {
            'bools': [
                ...
                },
                {
                    ...
                    'tip': '不管怎么说，这也太邪门了，没看懂代码是啥意思，所以先没写',
                    'category': 'game_config',
    ...
    def handle_game_sv6_common_request(self, request: Node) -> Node:
        ...
        if game_config.get_bool('use_information'):
            ...
            info.add_child(Node.string('param_str_1', '[f:0]歓迎来ZHAN窩'))
            msg = """
            這糸一屑測試,不要在意.
            [sz:60]SZ60 [sz:30]SZ30
            ...
            [br:10]br
            """
            info.add_child(Node.string('param_str_2', msg))
        ...
        for course in courses:
            ...
            info.add_child(Node.bool('season_new_flg', course['season_id'] in {1, 2, 3, 4, 6}))
            info.add_child(Node.s16('course_id', course.get('course_id', course.get('skill_level', -1))))
            info.add_child(Node.string('course_name',
                                       course.get('course_name', skill_levels.get(course.get('skill_level', -1), ''))))
            ...
            info.add_child(Node.s16('skill_name_id',
                                    course.get('skill_name_id', skill_name_ids.get(course.get('skill_level', -1), 0))))
            info.add_child(
            ...
            info.add_child(
                Node.u32_array(
                    ...
                    [
                        ...
                        0,  # 12: Any value
                        0,  # 13: Another medal, perhaps old score medal?
                        0,  # 14: Another grade, perhaps old score grade?
                        0,  # 15: Any value
                        0,  # 16: Any value
                        0,  # 17: Any value
    ...
    def handle_game_sv6_save_m_request(self, request: Node) -> Node:
        ...
        if request.child('track') is not None:
            ...
            stats = {
                'exscore': request.child_value('exscore'),
    ...
    def unformat_profile(self, userid: UserID, request: Node, oldprofile: Profile) -> Profile:
        ...
        if earned_extrack_energy is not None:
            newprofile.replace_int('extrack_energy', newprofile.get_int('extrack_energy') + earned_extrack_energy)
