...
from bemani.common import ValidatedDict, BroadcastConstants, VersionConstants, Time, ID, intish, Profile
...
class IIDXBistrover(IIDXCourse, IIDXBase):
    ...
    GAME_CHART_TYPE_B7: Final[int] = 0
    GAME_CHART_TYPE_N7: Final[int] = 1
    GAME_CHART_TYPE_H7: Final[int] = 2
    GAME_CHART_TYPE_A7: Final[int] = 3
    GAME_CHART_TYPE_L7: Final[int] = 4
    GAME_CHART_TYPE_B14: Final[int] = 5  # THere are no B14 charts in the game but this would be the id if there were
    GAME_CHART_TYPE_N14: Final[int] = 6
    GAME_CHART_TYPE_H14: Final[int] = 7
    GAME_CHART_TYPE_A14: Final[int] = 8
            ...
            'ints': [
                {
                    ...
                    'values': {
                        0: 'No Event',
                        1: '召しませ!BISTROVER Asia',
                        2: '召しませ!BISTROVER Europe',
                        3: '召しませ!BISTROVER Northern land',
                        4: '召しませ!BISTROVER North America',
                        5: '召しませ!BISTROVER Asia2',
                    }
                },
            ],
        }
    ...
    def handle_IIDX28shop_getname_request(self, request: Node) -> Node:
    ...
    def handle_IIDX28shop_savename_request(self, request: Node) -> Node:
    ...
    def handle_IIDX28shop_sentinfo_request(self, request: Node) -> Node:
        return Node.void('IIDX28shop')

    def handle_IIDX28shop_sendescapepackageinfo_request(self, request: Node) -> Node:
        root = Node.void('IIDX28shop')
        root.set_attribute('expire', str((Time.now() + 86400 * 365) * 1000))
        return root

    def handle_IIDX28shop_getconvention_request(self, request: Node) -> Node:
        root = Node.void('IIDX28shop')
        machine = self.data.local.machine.get_machine(self.config.machine.pcbid)
    ...
    def handle_IIDX28shop_setconvention_request(self, request: Node) -> Node:
        machine = self.data.local.machine.get_machine(self.config.machine.pcbid)
        if machine.arcade is not None:
            course = ValidatedDict()
    ...
    def handle_IIDX28ranking_getranker_request(self, request: Node) -> Node:
        # Expert mode is removed so do nothing
        return Node.void('IIDX28ranking')

    def handle_IIDX28music_crate_request(self, request: Node) -> Node:
    ...
    def handle_IIDX28music_getrank_request(self, request: Node) -> Node:
    ...
    def handle_IIDX28music_appoint_request(self, request: Node) -> Node:
    ...
    def handle_IIDX28music_reg_request(self, request: Node) -> Node:
        extid = int(request.attribute('iidxid'))
        musicid = int(request.attribute('mid'))
        game_chart = int(request.attribute('clid'))
        userid = self.data.remote.user.from_extid(self.game, self.version, extid)
    ...
    def handle_IIDX28music_play_request(self, request: Node) -> Node:
        musicid = int(request.attribute('mid'))
        game_chart = int(request.attribute('clid'))
        clear_status = self.game_to_db_status(int(request.attribute('cflg')))
        chart = self.game_to_db_chart(game_chart)
    ...
    def handle_IIDX28grade_raised_request(self, request: Node) -> Node:
        extid = int(request.attribute('iidxid'))
        ...
        eisei_data = request.child('eisei_data')
        if eisei_data is not None:
            if userid is not None:
                profile = self.get_profile(userid)
                if profile is None:
                    profile = Profile(self.game, self.version, '', extid)
                eisei_dict = profile.get_dict('eisei_data')
                eisei_dict.replace_int('clear_type', int(eisei_data.attribute('clear_type')))
                eisei_dict.replace_int('grade_id', int(eisei_data.attribute('grade_id')))
                eisei_dict.replace_int('grade_type', int(eisei_data.attribute('grade_type')))
                eisei_dict.replace_int('stage_num', int(eisei_data.attribute('stage_num')))
                eisei_dict.replace_int_array('past_achievement', 5, eisei_data.child_value('past_achievement'))
                eisei_dict.replace_int_array('past_selected_course', 5, eisei_data.child_value('past_selected_course'))
                eisei_dict.replace_int_array('max_past_achievement', 5, eisei_data.child_value('max_past_achievement'))
                eisei_dict.replace_int_array('max_past_selected_course', 5, eisei_data.child_value('max_past_selected_course'))
                profile.replace_dict('eisei_data', eisei_dict)
    ...
    def handle_IIDX28music_arenaCPU_request(self, request: Node) -> Node:
        root = Node.void('IIDX28music')
        music_list = [0, 0, 0, 0]
        cpu_list = [0, 0, 0, 0]
    ...
    def handle_IIDX28pc_common_request(self, request: Node) -> Node:
        ...
        extra_boss_event = Node.void('extra_boss_event')  # Always enable BISTRO LANDING Location 6
        root.add_child(extra_boss_event)
        extra_boss_event.set_attribute('phase', '5')
        return root

    def handle_IIDX28pc_delete_request(self, request: Node) -> Node:
        return Node.void('IIDX28pc')

    def handle_IIDX28pc_playstart_request(self, request: Node) -> Node:
        return Node.void('IIDX28pc')

    def handle_IIDX28pc_playend_request(self, request: Node) -> Node:
        return Node.void('IIDX28pc')

    def handle_IIDX28pc_visit_request(self, request: Node) -> Node:
        root = Node.void('IIDX28pc')
    ...
    def handle_IIDX28pc_shopregister_request(self, request: Node) -> Node:  # Not used anymore????
        root = Node.void('IIDX28pc')
        return root

    def handle_IIDX28pc_oldget_request(self, request: Node) -> Node:
    ...
    def handle_IIDX28pc_getname_request(self, request: Node) -> Node:
        refid = request.attribute('rid')
        userid = self.data.remote.user.from_refid(self.game, self.version, refid)
        if userid is not None:
            oldversion = self.previous_version()
    ...
    def handle_IIDX28pc_takeover_request(self, request: Node) -> Node:
        refid = request.attribute('rid')
    ...
    def handle_IIDX28pc_reg_request(self, request: Node) -> Node:
        refid = request.attribute('rid')
        name = request.attribute('name')
        pid = int(request.attribute('pid'))
    ...
    def handle_IIDX28pc_get_request(self, request: Node) -> Node:
        refid = request.attribute('rid')
    ...
    def handle_IIDX28pc_save_request(self, request: Node) -> Node:
        extid = int(request.attribute('iidxid'))
        self.put_profile_by_extid(extid, request)

        return Node.void('IIDX28pc')

    def handle_IIDX28pc_logout_request(self, request: Node) -> Node:
        return Node.void('IIDX28pc')

    def handle_IIDX28gameSystem_systemInfo_request(self, request: Node) -> Node:
        root = Node.void('IIDX28gameSystem')
    ...
    def handle_IIDX28pc_eaappliresult_request(self, request: Node) -> Node:
        clear_map = {
    ...
    def handle_IIDX28streaming_request(self, request: Node) -> Node:
        return Node.void('IIDX28streaming')
    ...
    def format_profile(self, userid: UserID, profile: Profile) -> Node:
        ...
        lightning_set_dict = profile.get_dict('lightning_setting')
        lightning_setting = Node.void('lightning_setting')
        root.add_child(lightning_setting)
        lightning_setting.set_attribute('headphone_vol', str(lightning_set_dict.get_int('headphone_vol')))
        lightning_setting.set_attribute('resistance_dp_left', str(lightning_set_dict.get_int('resistance_dp_left')))
        lightning_setting.set_attribute('resistance_dp_right', str(lightning_set_dict.get_int('resistance_dp_right')))
        lightning_setting.set_attribute('resistance_sp_left', str(lightning_set_dict.get_int('resistance_sp_left')))
        lightning_setting.set_attribute('resistance_sp_right', str(lightning_set_dict.get_int('resistance_sp_right')))
        lightning_setting.set_attribute('skin_0', str(lightning_set_dict.get_int('skin_0')))
        lightning_setting.set_attribute('flg_skin_0', str(lightning_set_dict.get_int('flg_skin_0')))
        lightning_setting.add_child(Node.s32_array('slider', lightning_set_dict.get_int_array('slider', 7)))
        lightning_setting.add_child(Node.bool_array('light', lightning_set_dict.get_bool_array('light', 6)))
        lightning_setting.add_child(Node.bool('concentration', lightning_set_dict.get_bool('concentration')))
        lightning_play_dict = profile.get_dict('lightning_play_data')
        lightning_play_data = Node.void('lightning_play_data')
        root.add_child(lightning_play_data)
        lightning_play_data.set_attribute('spnum', str(lightning_play_dict.get_int('spnum')))
        lightning_play_data.set_attribute('dpnum', str(lightning_play_dict.get_int('dpnum')))
        # Eisei Grade loading
        eisei_data_dict = profile.get_dict('eisei_data')
        eisei_grade = Node.void('eisei_grade_data')
        root.add_child(eisei_grade)
        detail = Node.void('detail')
        eisei_grade.add_child(detail)
        detail.set_attribute('grade_type', str(eisei_data_dict.get_int('grade_type')))
        detail.set_attribute('grade_id', str(eisei_data_dict.get_int('grade_id')))
        detail.set_attribute('stage_num', str(eisei_data_dict.get_int('stage_num')))
        detail.set_attribute('clear_type', str(eisei_data_dict.get_int('clear_type')))
        detail.add_child(Node.s32_array('past', eisei_data_dict.get_int_array('past_achievement', 5)))
        detail.add_child(Node.s32_array('max_past', eisei_data_dict.get_int_array('max_past_achievement', 5)))
        detail.add_child(Node.s32_array('selected_course', eisei_data_dict.get_int_array('past_selected_course', 5)))
        detail.add_child(Node.s32_array('max_selected_course', eisei_data_dict.get_int_array('max_past_selected_course', 5)))
        ...
        # 召しませ!BISTROVER event
        event1_dict = profile.get_dict('event_1')
        event1 = Node.void('event_1')
        root.add_child(event1)
        event1.set_attribute('event_play_num', str(event1_dict.get_int('event_play_num')))
        event1.set_attribute('last_select_area_id', str(event1_dict.get_int('last_select_area')))
        event1.set_attribute('story_prog', str(event1_dict.get_int('story_prog')))
        event1.set_attribute('failed_num', str(event1_dict.get_int('failed_num')))
        for area_data in achievements:
            if area_data.type != 'area_data':
                continue
            area_data_node = Node.void('area_data')
            event1.add_child(area_data_node)
            area_data_node.set_attribute('area_id', str(area_data.id))
            area_data_node.set_attribute('play_num', str(area_data.data.get_int('play_num')))
            area_data_node.set_attribute('area_prog', str(area_data.data.get_int('area_prog')))
            area_data_node.set_attribute('last_select_recipe', str(area_data.data.get_int('last_select_recipe')))
            area_data_node.set_attribute('operation_num', str(area_data.data.get_int('operation_num')))
            area_data_node.set_attribute('operation_prog', str(area_data.data.get_int('operation_prog')))
            area_data_node.set_attribute('recipe_prog0', str(area_data.data.get_int('recipe_prog0')))
            area_data_node.set_attribute('recipe_prog1', str(area_data.data.get_int('recipe_prog1')))
            area_data_node.set_attribute('recipe_prog2', str(area_data.data.get_int('recipe_prog2')))
            area_data_node.set_attribute('recipe_prog3', str(area_data.data.get_int('recipe_prog3')))
            area_data_node.set_attribute('recipe_prog4', str(area_data.data.get_int('recipe_prog4')))
            area_data_node.add_child(Node.bool('is_complete', area_data.data.get_bool('is_complete')))
        ...
        for i in range(8):
            extra_boss_event.set_attribute(f'boss_orb_{i}', str(extra_boss_event_dict.get_int(f'boss_orb_{i}')))
    ...
    def unformat_profile(self, userid: UserID, request: Node, oldprofile: Profile) -> Profile:
        ...
        if lightning_setting is not None:
            lightning_set_dict = newprofile.get_dict('lightning_setting')
            lightning_set_dict.replace_int('headphone_vol', int(lightning_setting.attribute('headphone_vol')))
            lightning_set_dict.replace_int('resistance_sp_left', int(lightning_setting.attribute('resistance_sp_left')))
            lightning_set_dict.replace_int('resistance_sp_right', int(lightning_setting.attribute('resistance_sp_right')))
            lightning_set_dict.replace_int('resistance_dp_left', int(lightning_setting.attribute('resistance_dp_left')))
            lightning_set_dict.replace_int('resistance_dp_right', int(lightning_setting.attribute('resistance_dp_right')))
            # lightning_set_dict.replace_int('skin_0', int(lightning_setting.attribute('skin_0')))
            # lightning_set_dict.replace_int('flg_skin_0', int(lightning_setting.attribute('flg_skin_0')))
            lightning_set_dict.replace_int_array('slider', 7, lightning_setting.child_value('slider'))
            lightning_set_dict.replace_bool_array('light', 6, lightning_setting.child_value('light'))
            lightning_set_dict.replace_bool('concentration', lightning_setting.child_value('concentration'))
            newprofile.replace_dict('lightning_setting', lightning_set_dict)
        lightning_play_data = request.child('lightning_play_data')
        if lightning_play_data is not None:
            lightning_play_dict = newprofile.get_dict('lightning_play_data')
            if cltype == self.GAME_CLTYPE_SINGLE:
                lightning_play_dict.increment_int('spnum')
            if cltype == self.GAME_CLTYPE_DOUBLE:
                lightning_play_dict.increment_int('dpnum')
            newprofile.replace_dict('lightning_play_data', lightning_play_dict)
        ...
        # 召しませ!BISTROVER Event Saving
        event1 = request.child('event_1')
        if event1 is not None:
            event1_dict = newprofile.get_dict('event_1')
            event1_dict.increment_int('event_play_num')
            event1_dict.replace_int('failed_num', int(event1.attribute('failed_num')))
            event1_dict.replace_int('story_prog', int(event1.attribute('story_prog')))
            last_select_area = int(event1.attribute('last_select_area_id'))
            # Save any area data, also see if we need to update last_select_area
            for area_data in event1.children:
                if area_data.name != 'area_data':
                    continue

                # Update the area's progress in the DB
                areaid = int(area_data.attribute('area_id'))
                play_num = int(area_data.attribute('play_num'))
                is_complete = area_data.child_value('is_complete') or False
                last_select_recipe = int(area_data.attribute('last_select_recipe'))
                operation_num = int(area_data.attribute('operation_num'))
                operation_prog = int(area_data.attribute('operation_prog'))
                recipe_prog0 = int(area_data.attribute('recipe_prog0'))
                recipe_prog1 = int(area_data.attribute('recipe_prog1'))
                recipe_prog2 = int(area_data.attribute('recipe_prog2'))
                recipe_prog3 = int(area_data.attribute('recipe_prog3'))
                recipe_prog4 = int(area_data.attribute('recipe_prog4'))
                ...
                self.data.local.user.put_achievement(
                    ...
                    {
                        'play_num': play_num,
                        'last_select_recipe': last_select_recipe,
                        'operation_num': operation_num,
                        'operation_prog': operation_prog,
                        'recipe_prog0': recipe_prog0,
                        'recipe_prog1': recipe_prog1,
                        'recipe_prog2': recipe_prog2,
                        'recipe_prog3': recipe_prog3,
                        'recipe_prog4': recipe_prog4,
                        'is_complete': is_complete,
                    }
                )

                # If they cleared this area, then we should be at least onto the next one.
                if is_complete:
                    last_select_area = max(last_select_area, areaid + 1)

            event1_dict.replace_int('last_select_area', last_select_area)
            newprofile.replace_dict('event_1', event1_dict)
        ...
        if extra_boss_event is not None:
            ...
            for i in range(8):
                extra_boss_event_dict.replace_int(f'boss_orb_{i}', int(extra_boss_event.attribute(f'boss_orb_{i}')))
