...
class IIDXHeroicVerse(IIDXCourse, IIDXBase):
    ...
    def handle_IIDX27pc_common_request(self, request: Node) -> Node:
        ...
        extra_boss_event = Node.void(
            "extra_boss_event"
        )  # Always enable SHADOW REBELLION 7
    ...
    def handle_IIDX27pc_shopregister_request(
        self, request: Node
    ...
    def format_profile(self, userid: UserID, profile: Profile) -> Node:
        ...
        event1.set_attribute(
            "last_select_gym_id", str(event1_dict.get_int("last_select_gym"))
        )
        for gym_data in achievements:
            ...
            gym_data_node.set_attribute(
                "play_num", str(gym_data.data.get_int("play_num"))
            )
            gym_data_node.set_attribute(
                "gauge_spirit", str(gym_data.data.get_int("gauge_spirit"))
            )
            gym_data_node.set_attribute(
                "gauge_technique", str(gym_data.data.get_int("gauge_technique"))
            )
            gym_data_node.set_attribute(
                "gauge_body", str(gym_data.data.get_int("gauge_body"))
            )
            gym_data_node.set_attribute(
                "boss_attack_num", str(gym_data.data.get_int("boss_attack_num"))
            )
            gym_data_node.set_attribute(
                "boss_damage", str(gym_data.data.get_int("boss_damage"))
            )
            gym_data_node.set_attribute(
                "disp_lounge_list", str(gym_data.data.get_int("disp_lounge_list"))
            )
            gym_data_node.set_attribute(
                "stb_type", str(gym_data.data.get_int("stb_type"))
            )
            gym_data_node.add_child(
                Node.bool("is_complete", gym_data.data.get_bool("is_complete"))
            )
            gym_data_node.add_child(
                Node.bool("is_gauge_max", gym_data.data.get_bool("is_gauge_max"))
            )
    ...
    def unformat_profile(
        ...
        if event1 is not None:
            ...
            event1_dict.replace_int(
                "event_play_num", event1_dict.get_int("event_play_num") + 1
            )
