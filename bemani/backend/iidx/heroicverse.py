...
class IIDXHeroicVerse(IIDXCourse, IIDXBase):
            ...
            'ints': [
                {
                    ...
                    'values': {
                        0: 'No Event',
                        1: 'HEROIC WORKOUT!! ROOM 1',
                        2: 'HEROIC WORKOUT!! ROOM 2',
                        3: 'HEROIC WORKOUT!! ROOM 3',
                        4: 'HEROIC WORKOUT!! ROOM 4',
                        5: 'HEROIC WORKOUT!! ROOM 5',
                        6: 'HEROIC WORKOUT!! ROOM 5.5'
                    }
                },
            ],
        }
    ...
    def handle_IIDX27music_arenaCPU_request(self, request: Node) -> Node:
        root = Node.void('IIDX27music')
        music_list = [0, 0, 0, 0]
        cpu_list = [0, 0, 0, 0]
    ...
    def handle_IIDX27pc_common_request(self, request: Node) -> Node:
        ...
        if event_phase == 0 or (self.omnimix and (not omni_events)):
            event1 = 0
            boss_phase = 0
        elif event_phase in [1, 2, 3, 4, 5, 6]:
            boss_phase = 1
        ...
        extra_boss_event = Node.void('extra_boss_event')  # Always enable SHADOW REBELLION 7
        root.add_child(extra_boss_event)
        extra_boss_event.set_attribute('phase', '6')
    ...
    def handle_IIDX27pc_shopregister_request(self, request: Node) -> Node:  # Not used anymore????
        root = Node.void('IIDX27pc')
        return root
    ...
    def unformat_profile(self, userid: UserID, request: Node, oldprofile: Profile) -> Profile:
        ...
        if event1 is not None:
            ...
            # Save any gym data, also see if we need to update last_select_gym
            for gym_data in event1.children:
                ...
                # If they cleared this gym, then we should be at least onto the next one.
                if is_complete:
