...
class DDRX3Client(BaseClient):
    ...
    def verify_game_load_m(self, ref_id: str) -> Dict[int, Dict[int, Dict[str, Any]]]:
        ...
        for child in resp.child("game").children:
            ...
            for typenode in child.children:
                ...
                self.assert_path(typenode, "type/score_2nd")
                self.assert_path(typenode, "type/rank_2nd")
                self.assert_path(typenode, "type/cnt_2nd")
                ...
                vals = {
                    ...
                    "score_2nd": typenode.child_value("score_2nd"),
                    "rank_2nd": typenode.child_value("rank_2nd"),
                    "cnt_2nd": typenode.child_value("cnt_2nd"),
                }
    ...
    def verify_game_save_m(self, ref_id: str, score: Dict[str, Any]) -> None:
        if score["score"] == 0 and score["score_2nd"] == 0:
            raise Exception("Must store either 2ndMIX or regular score!")
        if score["score"] != 0 and score["score_2nd"] != 0:
            raise Exception("Must store either 2ndMIX or regular score!")
        call = self.call_node()
        game = Node.void("game")
        ...
        data.set_attribute("playmode", str(1 if score["score"] > 0 else 5))
        data.set_attribute("rank_2nd", str(score["rank_2nd"]))
        data.set_attribute("score_2nd", str(score["score_2nd"]))
        data.set_attribute("combo_2nd", str(score["combo_2nd"]))
                ...
                for score in dummyscores:
                    ...
                    if score["score_2nd"] != 0:
                        if data["score_2nd"] != expected_score_2nd:
                            raise Exception(
                                f'Expected a score_2nd of \'{expected_score_2nd}\' for song \'{score["id"]}\' chart \'{score["chart"]}\' but got score_2nd \'{data["score_2nd"]}\''
                            )
                        if data["rank_2nd"] != expected_rank_2nd:
                            raise Exception(
                                f'Expected a rank_2nd of \'{expected_rank_2nd}\' for song \'{score["id"]}\' chart \'{score["chart"]}\' but got rank_2nd \'{data["rank_2nd"]}\''
                            )
