...
class IIDXRootageClient(BaseClient):
    ...
    def verify_iidx26pc_save(self, extid: int, card: str, lid: str, expert_point: Optional[Dict[str, int]]=None) -> None:
        call = self.call_node()
    ...
    def verify(self, cardid: Optional[str]) -> None:
        ...
        if cardid is None:
            ...
            self.verify_iidx26pc_save(profile['extid'], card, lid, {'course_id': 1, 'n_point': 0, 'h_point': 500, 'a_point': 0})
            profile = self.verify_iidx26pc_get(ref_id, card, lid)
            ...
            self.verify_iidx26pc_save(profile['extid'], card, lid, {'course_id': 1, 'n_point': 0, 'h_point': 1000, 'a_point': 0})
            profile = self.verify_iidx26pc_get(ref_id, card, lid)
            if sorted(profile['expert_point'].keys()) != [1]:
                raise Exception('Got back wrong number of expert course points!')
            if profile['expert_point'][1] != {'n_point': 0, 'h_point': 1000, 'a_point': 0}:
                raise Exception('Got back wrong expert points after saving!')
            self.verify_iidx26pc_save(profile['extid'], card, lid, {'course_id': 2, 'n_point': 0, 'h_point': 0, 'a_point': 500})
            profile = self.verify_iidx26pc_get(ref_id, card, lid)
            ...
            self.verify_iidx26music_breg(profile['extid'], {
                'id': 1000,
                'clear_status': 4,
                'pgnum': 123,
                'gnum': 123,
