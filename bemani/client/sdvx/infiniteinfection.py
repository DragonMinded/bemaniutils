...
class SoundVoltexInfiniteInfectionClient(BaseClient):
    ...
    def verify_game_save(self, location: str, refid: str, packet: int, block: int, blaster_energy: int, appealcards: List[int]) -> None:
        call = self.call_node()
    ...
    def verify(self, cardid: Optional[str]) -> None:
        ...
        if cardid is None:
            ...
            self.verify_game_save(location, ref_id, packet=0, block=0, blaster_energy=0, appealcards=[1001, 1002, 1003, 1004, 1005])
            profile = self.verify_game_load(card, ref_id, msg_type='existing')
